...
export default function App() {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        setLoading(true);
        const [accountsRes, batchesRes, employeesRes] = await Promise.all([
          fetch(`${API_URL}/bank-accounts`),
          fetch(`${API_URL}/batches`),
          fetch(`${API_URL}/employees`),
        ]);

        if (!accountsRes.ok || !batchesRes.ok || !employeesRes.ok) {
        ...
        // We map `balance` to both `balance` and `initialBalance`
        // as your backend entity only has one balance field.
